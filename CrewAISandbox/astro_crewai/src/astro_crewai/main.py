...
from astro_crewai.crew import AstroCrewAI
...
def run():
    ...
    Run the crew.
    """
    inputs = {"topic": "Artimis II Mission", "current_year": str(datetime.now().year)}

    try:
        AstroCrewAI().crew().kickoff(inputs=inputs)
...
def replay():
    ...
    """
    try:
        AstroCrewAI().crew().replay(task_id=sys.argv[1])
